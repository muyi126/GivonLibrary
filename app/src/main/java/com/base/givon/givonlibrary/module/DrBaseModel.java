...
import com.base.givon.givonlibrary.api.DrBaseApi;
...
import com.base.givon.givonlibrary.common.transformer.BaseApiTransformer;
...
public class DrBaseModel extends BaseModel<DrBaseApi, DrBaseModel> {
    ...
    //校验验证码
    public Observable<Verify> checkVerifyCode(String mobileNumber, String code) {
        return getService().verifyCode("checkVerifyCode", mobileNumber, code).compose(new BaseApiTransformer<Verify>());
    }
    ...
    //获取首页订单数量
    public Observable<OrderCountEntity> getOrderCount(String token) {
        return getService().order("getOrderCount", token).compose(new BaseApiTransformer<OrderCountEntity>());
    }
    ...
    //获取消息列表
    public Observable<OrderMessageEntity> findMessageList(String token, int pageNo) {
        return getService().findMessageList("findMessageList", token, pageNo).compose(new BaseApiTransformer<OrderMessageEntity>());
    }
