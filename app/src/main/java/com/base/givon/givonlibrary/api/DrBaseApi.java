...
import com.base.givon.givonlibrary.api.entity.OrderCountEntity;
import com.base.givon.givonlibrary.api.entity.OrderEntity;
import com.base.givon.givonlibrary.api.entity.OrderListEntity;
import com.base.givon.givonlibrary.api.entity.OrderMessageEntity;
import com.base.givon.givonlibrary.api.entity.element.User;
import com.base.givon.givonlibrary.api.entity.element.Verify;
...
//    Observable<HttpResult<String>> verifyCode(@Query("method") String method, @Query("mobileNumber") String mobileNumber, @Query("code") String code);
    ...
    @FormUrlEncoded
    @POST("back/verifyCode.do")
    Observable<HttpResult<Verify>> verifyCode(@Field("method") String method, @Field("mobileNumber") String mobileNumber, @Field("code") String code);

    //注册 register 登录 login
    @FormUrlEncoded
    @POST("back/api/driver/base/base.do")
    Observable<HttpResult<User>> userAction(@Field("method") String method, @Field("name") String name, @Field("mobileNumber") String mobileNumber, @Field("code") String code, @Field("password") String password);

    //获取首页订单数量 getOrderCount
    @FormUrlEncoded
    @POST("back/api/driver/common/order.do")
    Observable<HttpResult<OrderCountEntity>> order(@Field("method") String method, @Field("token") String token);

    //获取司机信息 getDriverInfo(token)
    @FormUrlEncoded
    @POST("back/api/driver/common/account.do")
    Observable<HttpResult<User>> account(@Field("method") String method, @Field("token") String token, @Field("password") String password, @Field("newPassword") String newPassword);
    ...
    @FormUrlEncoded
    @POST("back/api/driver/common/order.do")
    Observable<HttpResult<OrderListEntity>> findOrderList(@Field("method") String method, @Field("token") String token, @Field("pageNo") int pageNo, @Field("dateType") int dateType, @Field("tripDate") String tripDate, @Field("isComplete") int isComplete);


    //获取订单详情
    @FormUrlEncoded
    @POST("back/api/driver/common/order.do")
    Observable<HttpResult<OrderEntity>> viewDriverOrder(@Field("method") String method, @Field("token") String token, @Field("driverOrderId") String driverOrderId);
    ...
    Observable<HttpResult<OrderMessageEntity>> findMessageList(@Query("method") String method, @Query("token") String token, @Query("pageNo") int pageNo);
