...
import com.base.givon.baseobj.common.widget.BaseDialog;
...
public class UiUtil {
    ...
    public static int dip2px(float dpValue) {
        final float scale = GivApp.getInstance().getResources().getDisplayMetrics().density;
    ...
    public static void exitToLogin(String loginClassName) {
        ExitUtil.getInstance().ExitApp();
        Intent intent = null;
        if (StringUtil.isEmpty(loginClassName)) {
            return;
        }
        try {
            intent = new Intent(GivApp.getInstance(), Class.forName(loginClassName));
            intent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
            GivApp.getInstance().startActivity(intent);
    ...
    /**
     * 可以通过ActLoginActivity.class.getName()获取到classname
     * @param loginClassName
     */
    public static void showLogOutDialog(final String loginClassName) {
